...
(function(root, factory) {
  ...
  function defineLazyP5Property(nome, getter) {
    Object.defineProperty(p5.prototype, nome, {
      configurable: true,
      enumerable: true,
      get: function() {
        ...
        if (!(nome in context._p5PlayProperties)) {
          context._p5PlayProperties[nome] = getter.call(context);
        }
        return context._p5PlayProperties[nome];
      }
    });
  }
...
*/
  ...
  */
   ...
   *
   * An animation can be created either by passing a series of file nomes,
   * no matter how many or by passing the first and the last file nome
   * of a numbered sequence.
   * p5.play will try to detect the sequence pattern.
   *
   * For example if the given filenomes are
  ...
  function Animation(pInst) {
    ...
    if (
      ...
      if (ext1 !== ".png") {
        pInst.print(
          "Animation error: you need to use .png files (filenome " + from + ")"
        );
        from = -1;
      }
      ...
      if (ext2 !== ".png") {
        pInst.print(
          "Animation error: you need to use .png files (filenome " + to + ")"
        );
        to = -1;
      }
  ...
  function SpriteSheet(pInst) {
     ...
     * @param frame_nome  Can either be a string nome, or a numeric index.
     ...
     * @method drawFrame
     */
    this.drawFrame = function(frame_nome, x, y, width, height) {
      var frameToDraw;
      if (typeof frame_nome === "number") {
        frameToDraw = this.frames[frame_nome].frame;
      } else {
        for (var i = 0; i < this.frames.length; i++) {
          if (this.frames[i].nome === frame_nome) {
